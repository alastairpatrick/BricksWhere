...
class BackgroundTask(QObject):
    """Runs a background function in a thread pool, while updating GUI via signals.
    ...
    def run(self, fn: Callable, *args, **kwargs) -> Future:
        ...
        self._future = self._executor.submit(lambda: fn(progress, self.is_cancelled, *args, **kwargs))
        self._timer.start()
